...
- **GameShift no longer registers a SYSTEM boot task for a file placed next to it** - on every start, GameShift checked its own folder for `GameShift.Watchdog.exe` and, if one was there, registered a scheduled task that ran it as SYSTEM at every boot. Releases never include that file, so a normal install was unaffected, but GameShift is often run from a folder that other users or programs can write to, and a file planted there would have gained full control of the system after GameShift's next start. The registration is removed until the watchdog ships through an admin-only install. The README and security policy now also recommend keeping `GameShift.App.exe` in a folder only administrators can change, such as `C:\Program Files\GameShift`.
...
- **Removed launcher games stay removed** - removing a game that a Steam, Epic, GOG, or Xbox library scan found only lasted until the next scan, which added it back to the Game Library and let GameShift detect and optimize it again. Removed launcher games are now remembered, so later scans and restarts no longer add them back, and the removal prompt says so. A title that also has a built-in GameShift profile (such as Apex Legends or Fortnite) can still be detected through that profile. If the change cannot be saved, the removal is reported as failed and the game stays in the library. Manually added games can still be removed and added again freely. Thanks to @YI-WebDev for reporting this and contributing the fix.
- **Anti-cheat-protected games are detected even when their process cannot be inspected** - some kernel anti-cheat games, such as Apex Legends with Easy Anti-Cheat, block GameShift from reading their process's file path, and when GameShift needed that path to identify the game, the game could go undetected. GameShift now asks for the path with the minimum access rights, tells a running but unreadable process apart from one that has already exited, and for Apex Legends, whose path stays unreadable, falls back to its process name only after confirming the process is still running under that name. This covers games started while GameShift is running; an Apex Legends session that was already running when GameShift started is still not picked up. Thanks to @YI-WebDev for reporting this.

### Changed

- **Minecraft: Java Edition is no longer detected from any `javaw.exe`** - every Java desktop application runs as `javaw.exe`, so matching on that name alone could optimize (and later revert around) an unrelated program such as an IDE. Minecraft Java is therefore no longer detected automatically; you can still add it manually in the Game Library.
